...
class DatabaseStore implements CacheStore
{
    ...
    function  __construct ()
    {
        if (is_null(self::$db))
        {
            $_c = tt_config('cache.database');

            self::$table = $_c['table'];

            try
            {
              self::$db = new \PDO("mysql:host={$_c['host']};dbname={$_c['dbname']};charset={$_c['charset']}",$_c['user'],$_c['password']);
            }
            catch (\PDOException $e)
            {
              throw new \Exception("Cache DatabaseStore Exception: ".$e->getMessage());
            }
